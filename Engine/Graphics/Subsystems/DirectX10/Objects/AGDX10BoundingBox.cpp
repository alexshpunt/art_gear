...
AGDX10BoundingBox::AGDX10BoundingBox( D3DXVECTOR3 v1, D3DXVECTOR3 v2, ID3D10Device* device ) : AGDX10Primitive( device )
{
	...
	/*buffDesc.ByteWidth = sizeof( m_indices );
	buffDesc.BindFlags = D3D10_BIND_INDEX_BUFFER; 
	subresData.pSysMem = m_indices;

	hr = device->CreateBuffer( &buffDesc, &subresData, &m_indexBuffer );*/
}

AGDX10BoundingBox::~AGDX10BoundingBox()
{

}

void AGDX10BoundingBox::draw(ID3D10Device* device, AGDX10Camera* camera)
{
...
float AGDX10BoundingBox::intersect( D3DXVECTOR3 rayOrigin, D3DXVECTOR3 rayDir )
{
	float retDist = -1.0f; 
	for( int i = 0; i < 34; i++ )
	{
