...
const wstring& AGDXMaterial::getTexture( int index ) const
{
	if( index >= 0 && index < 12 )
...
void AGDXMaterial::loadFrom( ifstream& in, ID3D10Device* device )
{
	...
	vector< AGDXTextureResource > textResources; 
	for( int i = 0; i < 12; i++ )
	{
	...
	for( int i = 0; i < 12; i++ )
	{
		...
		if( texture != L"NULL" )
		{
			AGDXTextureResource* textResource = new AGDXTextureResource; 
...
void AGDXMaterial::setWorldMatrix(D3DXMATRIX world)
{
	m_worldVar->SetMatrix( world );
}

void AGDXMaterial::setViewMatrix(D3DXMATRIX view)
{
	m_viewVar->SetMatrix( view );
}

void AGDXMaterial::setProjMatrix(D3DXMATRIX proj)
{
	m_projectionVar->SetMatrix( proj );
}

void AGDXMaterial::apply( ID3D10Device* device )
{
